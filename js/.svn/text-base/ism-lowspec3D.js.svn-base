...
// this version is for CIRCLE seats
    ...
    function parseData() {
              ...
              function storeSeat( seatClass, seatSpecial ){  
                      ...
                      if (seatClass == "seat-unavail") { // immediately quit if unavailable seat!                
                        storedSeats += '<circle class="seat '+ seatClass +'"  cx="'+ seatX +'" cy="'+ seatY +'" r="1.3"/>';
                        return;
                      }   
                      ...
                      /* SVG - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
                      storedSeats += '<circle class="seat '+ seatClass +'" data-section="'+ sectId +'"  data-row="'+ rowId +'"  data-seat="'+ seatId +'" data-distance="'+ dist +'" cx="'+ seatX +'" cy="'+ seatY +'" r="1.3"/>'; // fill="#DDDDDD"
 ...
 function addSmoothZoom(){
  ...
  function updateSpotLocation(zoomData){
    var centerDiff = 7
        adjX = zoomData.centerX/10 - centerDiff,
        adjY = zoomData.centerY/10 - centerDiff
...
/*
  function enable3D(){
    var elStr = $("#map1a").css("transform");
    elStr = elStr.substring(elStr.indexOf("(")+1, elStr.indexOf(")")).split(",");
    
    var matrixVal = "matrix3d(" + 
    elStr[0] + ",0,0,0,0," + 
    elStr[3] + ",0,0,0,0," + 
    "1,0," + elStr[4] + "," + elStr[5] + ",0,1)";
    //$("#map1a").css("transform","none");
    //console.log(elStr);
    $("#map1a").css({
      transform: "none",
      visibility: "hidden"
