 ...
 // function sortBest(a,b){ return $(a).data('best') - $(b).data('best');  }
  ...
  function storeListing(){
	...
	function storeListItem(seatClass, sectId, rowId, seatId, seatPrice, dist, seatSpecial, sectName, startSeat, endSeat, groupId, cx, cy){
		...
		storedList += '<li class="picks-item gotoLink" data-link="review-page" data-price="'+ seatPrice +'" data-seatstart="'+ startSeat+'" data-seatend="'+ endSeat+'">'
		...
		storedList += '<img src="img/v2/mini-map-indicator.png" class="minimap-indicator" style="left:'+ cx + '; top:' +cy+';">';    
		storedList += '</div>';      
		storedList += '</div>';
    ...
    storedList += '<span class="seat-dist hide">'+ dist +'</span>';
  ...
  /* Fri, Aug 1 @ 6:53pm
  		finish seat hover/ data transmission
  		todo: FIX seat hover toggle classes when selecting another seat
  		try: bundling hideSeatHover() inside hideSeatHover() or visa versa
  ...
  function updateSpotLocation(){
    ...
    // sort : highest - lowest
		...
		function centerSeat( seat ){
					  	...
					  	if ( !animFrame ) {
					  		...
					  		var desZoom = maxZoom, // @ 169, circles have width of 44px
					  				...
					  				zoomY			= bbox.y - (62/desZoom * 100); //95
		...
		function addToPreCart( seat ){
							...
							//hideSeatHover();
							hideSeatHover();
							//$pcLI.clone().appendTo($pcList).removeClass("clone-li");

							var newLi = '<li class="cart-item gotoLink" data-link="review-page">';
									newLi +='<div class="item-img"><img class="seat-view" src="img/seatview/208_SeatViews_640x427.jpg"></div>';
									newLi += '<div class="item-info"><span class="seat-resale">Resale</span><span class="seat-sec">Promenade 208</span><span class="seat-row">3</span> <span class="seat-num">2</span> <span class="seat-dist hide">200</span> <span class="pc-id hide">200</span> <a href="#" class="details-link">Details</a> </div>';
                	newLi += '<div class="item-price-edit"><a href="#" class="remove-link"><span class="item-price">226.00</span>Remove</a></div></li>';
		...
		function addToCart(pickItem){
					...
					var newLi = '<li class="cart-item">';
							...
							newLi += filter_tixQty +' Tickets <a href="#" class="details-link">Details</a></span>';
							newLi += '<span class="item-details">	'+ sect +'<br>Row '+ row +', Seats '+ startSeat +'-'+ endSeat +'</span></div>';
					...
					var subtotal = 0;
					$(".cart-page .cart-list .item-price").each(function(){
					...
					return;
 					...
 					if ($bagItems.length > 0) {
						 		$bagItems.each(function(index){
								 				...
								 				// retrieve data
								 				var imgSrc 	= $(".seat-vew", $self).attr('src'),
								 						sect 		= $(".seat-sec", $self).text(),
								 						row 		= $(".seat-row", $self).text(),
								 						num 		= $(".seat-num", $self).text(),
								 						price 	= $(".item-price", $self).text();


								 				// find if other sect/row exists
								 				// returns other items with common
								 				var $checkSect = $bagItems.find(".seat-sec").slice( index + 1).filter(function(){ return this.innerHTML === sect;}).parents(".cart-item"),
								 						$checkRow = $bagItems.find(".seat-row").slice( index + 1).filter(function(){ return this.innerHTML === row;}).parents(".cart-item"),
								 						$checkNum = $bagItems.find(".seat-num").slice( index + 1).filter(function(){ return this.innerHTML === num;}).parents(".cart-item");

								 				// if $checkSect and $checkRow matches
								 				if ( $checkSect.is($checkRow) ) {
		...
		$("body").on("click", ".btn-add", function(e){
			...
			var $pickItem = $(this).parents(".picks-item");

					if ($pickItem.length == 0)  $pickItem = $(this).parents(".review-page");
	 ...
	 function addSectionClicks() {
	  ...
	  $sectionAvail.bind("click tap",function(e) {			  	

			  	var is_coversOn = $svgMap.svgHasClass("covers-on"),
			  			thisID = $(this).attr("id"),
			  				bbox = document.getElementById(thisID).getBBox();
		 ...
		 $svgShell.bind("click tap",function(e) { 
	...
	function addPicksPins() {
  	...
  	$(".pin-top-pick").bind("click tap",function() {
			if (zoomRatio < coversThreshold || iPad) { displayInfo($(this),0,"pin"); }
  	}).bind("mouseenter",function() {
			if (zoomRatio >= coversThreshold) { displayInfo($(this),1,"pin"); }
  	});
		*/
	}
