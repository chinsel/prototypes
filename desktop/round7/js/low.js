$(function() {
  ...
  function filtersText() {
  	$(".filters-additional").each(function() {
	  	...
	  	if (!text) { text = "None"; }
	  	/*if (!text && $(this).hasClass("filters-show-only")) { text = "All"; }
	  	else if (!text) { text = "None"; }*/
	  	$(".filters-main legend").filter(function(){return this.innerHTML == legend}).next().html("<span>"+text);
  	});
  }
  //filtersText();
	...
	$(".filters-all .has-lock label").mouseenter(function() {
		var $filter = $(this);
	...
	// set height and scroll of more filters
	$(".filters-all").css("height",$(".filters-main").height()).show();
	if (!iPad) { $(".filters-all").jScrollPane(); }

	// toggle top picks
	$(".top-picks .hdr").bind("click",function(e) {
		if (alt == "2") { e.stopPropagation(); }
		$(this).parent().toggleClass("collapsed")
			.toggleClass("expanded")
  ...
  function zoomInfo(zoomData) {
	...
	function onPanComplete(zoomData) {
		...
		if (!seatLevel && zoomRatio >= showSeats && $(".top-picks .toggle").hasClass("expanded")) { seatLevel = 1; setTimeout(function() { $(".top-picks .hdr").trigger("click"); },0); }
  ...
  function changeBubbles() {
		if (zoomRatio >= (showSeats - minZoom) / 5 + minZoom && zoomRatio < showSeats) {
			$(".bubble .price.lowest").hide();
			$(".bubble .price.range").show();
		} else if (zoomRatio < (showSeats - minZoom) / 5 + minZoom && zoomRatio < showSeats) {
	...
	function applyToMap() {
		...
		for (i=0;i<$seatAvail.length;i++) {
			var $thisSeat = $seatAvail.eq(i),
					...
					shortLabel = (!label) ? null : label.split(" ")[0],
					$siblings = (type == ".seat-standard") ? $thisSeat.nextUntil(".seat:not('.seat-standard')",".seat-standard[data-section='"+section+"'][data-row='"+row+"']").addBack() : $thisSeat.nextUntil(".seat:not('.seat-resale')",".seat-resale[data-section='"+section+"'][data-row='"+row+"'][data-price='"+price.toFixed(2)+"']").addBack(),
					isTopPick = 0;
			var include = ($("label").filter(function(){return this.innerHTML.indexOf('Standard') != -1;}).prev().is(":checked") && type == ".seat-standard" && !$("label").filter(function(){return this.innerHTML.indexOf(shortLabel) != -1;}).length);
			if (!include) { include = ($("label").filter(function(){return this.innerHTML.indexOf('Resale Seats') != -1;}).prev().is(":checked") && type == ".seat-resale"); }
			if (!include)	{ include = (!!$("label").filter(function(){return this.innerHTML.indexOf(shortLabel) != -1;}).prev().is(":checked")); }
			i += $siblings.length - 1;
  ...
  function addTopPicks() {
  	...
  	// interaction with map pins
		if (!iPad) {
	  	$(".top-item").mouseenter(function() {
	  		...
	  		var label = $(this).data("label"),
	  				...
	  				$hover = $(".hover-top-pick[data-label='"+label+"']");
	  		if (zoomRatio < showSeats) {
		  		$pin.removeClass("hover");
		  		$hover.removeClass("on");
		  	}
	  	}).bind("vclick",function() {
  			$(".pin-top-pick .pin").removeClass("hover");
	  		$(".hover-top-pick").removeClass("on");
	  	});
  	} else if (alt == "1") {
  		$(".top-item .info").bind("vclick",function(e) {
	  		e.stopPropagation();
		...
		if (alt == "2") {
  		...
  		$(".top-item .info").bind("vclick",function(e) {
	  		e.stopPropagation();
  			clearTimeout(hoverTimerAlt2);
  			$(".pin-top-pick .pin").removeClass("hover");
  			$(".hover-top-pick").removeClass("on");
	  		var label = $(this).parent().data("label"),
	  				zoomX = $(".pin-top-pick[data-label='"+label+"']").data("position").split(",")[0],
	  				zoomY = $(".pin-top-pick[data-label='"+label+"']").data("position").split(",")[1];
	  		$map.smoothZoom("focusTo",{x: zoomX, y: zoomY, zoom: showSeats + ((iPad) ? 50 : 20), speed: 20});
	  		$(".hover-top-pick[data-label='"+label+"']").addClass("on");
	  		hoverTimerAlt2 = setTimeout(function() { $(".hover-top-pick[data-label='"+label+"']").removeClass("on"); },3000);
  		});
  	}
	...
	function addScrollToPicks() {
		...
		picksTotal = $(".top-item.on").not(".carted").length;
		...
		// vertical scroll to move through picks
		$(".no-touch .top-picks .container").bind("mousewheel",function(e) {
  			//console.log(e.originalEvent.wheelDeltaY);
  			$(this).data("jsp").scrollByX(e.originalEvent.wheelDeltaY * (-1));
		});
	...
	function addPicksPins() {
  	...
  	$(".pin-top-pick").bind("tap",function(e) {
  		e.stopPropagation();
			$(".pin-top-pick .pin").removeClass("hover");
  		$(".hover-top-pick").removeClass("on");
			if (iPad) { displayInfo($(this),0,"pin"); }
  	}).bind("mouseenter",function() {
			displayInfo($(this),1,"pin");
	...
	function displayInfo($el,hover,type) {
  	...
  	if (elTop < containerHt / 2) {
  		...
  		$click.removeClass("above").css({"top":elTop + elHt - ((iPad && !portrait) ? 0 : 0) + 9 + "px","left":elLeft + elWt / 2 - clickWt / 2 + "px","bottom":""});
  		$(".caret",$click).addClass("up").removeClass("down");
  	} else {
  		// place info above pin/seat
  		$click.addClass("above").css({"bottom":topPicksHt + containerHt - elTop + ((iPad && !portrait) ? 0 : 0) + 9 + "px","left":elLeft + elWt / 2 - clickWt / 2 + "px","top":""});
  	...
  	$(".add",$click).filter(function(){return this.innerHTML == 'Add to Cart'}).unbind().bind("vclick",function(e) {
  		e.stopPropagation();
	  	addToCart(0,label);
  	});
  	$(".add",$click).filter(function(){return this.innerHTML == 'Select Seat'}).unbind().bind("vclick",function(e) {
  		e.preventDefault();
  		e.stopPropagation();
  		$el.trigger("click");
  		$click.removeClass("on");
  	});
  	$(".info",$click).unbind().bind("click tap",function(e) {
  		e.preventDefault();
  		e.stopPropagation();
  		if (pin) { $el = $(".top-pick[data-label='"+label+"']"); }
  ...
  function displayDetails($el,topPick,pin,inbag) {
  	...
  	for (i=0;i<items;i++) {
	  	...
	  	detailsStr += '<li class="details-item'+((i === current) ? ' current' : '')+'"'+((pin) ? 'data-label="'+label+'"' : '')+'><div class="images"><img src="../../mobile/img/seatview/'+section+'_SeatViews_640x427.jpg" class="main-img" /><ul class="img-list"><li class="current"><img src="../../mobile/img/seatview/'+section+'_SeatViews_640x427.jpg" /></li><li><!--<b class="polling"></b>--><img src="" width="640" height="427" /></li></ul><img src="" class="seat-level-view" width="640" height="427" /></div><div class="desc"><div><p class="info">'+((topPick) ? '<span class="title">'+label+'<br /></span>' : '')+((resale) ? '<span class="label-resale">Resale<br /></span>' : '')+'<span class="section">'+sectionName+'</span><br /><span class="row-seat"><span class="row">Row '+row+'</span>, <span class="seat">'+((pin) ? seat+' seats in this row' : 'Seat '+seat)+'</span></span><br />'+((pin) ? '<span class="add-note">Add to cart to see seats<br /></span>' : '')+'<span class="hdr">Details</span><span class="dist">&bull; About '+distance+' ft from stage</span></p><p class="price">'+((pin) ? '<span class="each">$'+price+' x '+qty+'<br /></span>' : '')+'<span class="total">$'+total+'</span></p></div><div class="mobile">Available for <span>FREE</span> mobile delivery!</div>'+((pin) ? '<a href="#cart" class="add">Add to Cart</a>' : '<a href="#" class="add select">Select Seat</a>')+'</div></li>';
  	...
  	var $detailsContainer = $(".details .container");
		$detailsContainer.jScrollPane();
  	if (items > 1) {
	  	$detailsContainer.data("jsp").scrollToX((detailWt + 10) * current);
  	}
  	if (!iPad) {
	  	var lastScrollX = (detailWt + 10) * current,
	  			currentScrollX = lastScrollX;
	  	detailsInterval = setInterval(function() {
	  		if (lastScrollX != currentScrollX) {
	  			lastScrollX = currentScrollX;
	  		} else if (currentScrollX % (detailWt + 10) !== 0) {
	  			current = Math.round(currentScrollX / (detailWt + 10));
	  			$detailsContainer.data("jsp").scrollToX((detailWt + 10) * current,true);
	  			$(".details-item.current").removeClass("current");
					$(".details-item").eq(current).addClass("current");
	  			$(".details .pagination li.current").removeClass("current");
					$(".details .pagination li").eq(current).addClass("current");
	  		} else if (Math.round(currentScrollX / (detailWt + 10)) === 0 || Math.round(currentScrollX / (detailWt + 10)) === items - 1) {
	  			current = Math.round(currentScrollX / (detailWt + 10));
	  			$(".details-item.current").removeClass("current");
					$(".details-item").eq(current).addClass("current");
	  			$(".details .pagination li.current").removeClass("current");
	  	...
	  	$detailsContainer.bind("jsp-scroll-x",function(event, scrollPositionX, isAtLeft, isAtRight) {
	  		currentScrollX = parseInt(scrollPositionX);
			});
	  } else {
	  	var lastTouchX;
	  	$detailsContainer.bind("touchstart",function(e) {
	  		lastTouchX = e.originalEvent.touches[0].clientX;
			}).bind("touchend",function(e) {
	  		var currentTouchX = e.originalEvent.changedTouches[0].clientX;
	  		if (lastTouchX > currentTouchX && current < items - 1) {
	  			if (Math.abs(lastTouchX - currentTouchX) > (detailWt + 10) / 2) { current++; }
		  		$detailsContainer.data("jsp").scrollToX((detailWt + 10) * current,true);
	  			$(".details-item.current").removeClass("current");
					$(".details-item").eq(current).addClass("current");
	  			$(".details .pagination li.current").removeClass("current");
					$(".details .pagination li").eq(current).addClass("current");
	  		} else if (lastTouchX < currentTouchX && current > 0) {
	  			if (Math.abs(lastTouchX - currentTouchX) > (detailWt + 10) / 2) { current--; }
		  		$detailsContainer.data("jsp").scrollToX((detailWt + 10) * current,true);
	  			$(".details-item.current").removeClass("current");
					$(".details-item").eq(current).addClass("current");
  	...
  	$(".details .pagination li").unbind().bind("click",function() {
  		...
  		$detailsContainer.data("jsp").scrollToX((detailWt + 10) * (thisIndex),true);
			$(".details-item.current").removeClass("current");
  	...
  	/*$(".details .images").each(function() {
  		var thisWt = $(this).width(),
  				thisHt = thisWt * 427 / 640;
  		$(".main-img",this).attr("width",thisWt).attr("height",thisHt);
  	});*/
  	$(".details .img-list li").unbind().bind("click",function() {
  	...
  	/*if (items > 1 && iPad) {
		  $detailsContainer.bind("swipeleft",function() {
				current++;
				$detailsContainer.data("jsp").scrollToX((detailWt + 10) * current,true);
  			$(".details-item.current").removeClass("current");
				$(".details-item").eq(current).addClass("current");
  ...
  function addSectionClicks() {
	...
	function addSeatHovers() {
		$seatAvail.bind("mouseenter tap",function(e) {
  		e.stopPropagation();
  ...
  function addToBag($el) {
  	...
  	$(".bag-item:last .details-link").bind("click",function(e) {
  		e.stopPropagation();
  ...
  function addToCart(bag,label) {
  	...
  	// build or add to cart list
		for (i=0;i<cartItems;i++) {
			...
			cartStr += '<li class="'+classes+'" '+((bag) ? '' : 'data-label="'+label+'"')+'><img src="../../mobile/img/seatview/'+$item.data("section")+'_SeatViews_640x427.jpg" class="image" /><div class="desc"><p class="info">'+((type == ".seat-resale") ? '<span class="label-resale">Resale<br /></span>' : '')+'<span class="quantity">'+quantity+' Ticket'+((quantity > 1) ? 's' : '')+'<br /></span><span class="section">'+$item.data("section-name")+'</span><br /><span class="row-seat"><span class="row">Row '+row+'</span>, <span class="seat">Seat'+((quantity > 1) ? 's' : '')+' '+((quantity > 1) ? $item.data("seat")+"-"+$itemGroup.last().data("seat") : $item.data("seat"))+'</span></span></p><p class="price"><span class="total">$'+groupTotal.toFixed(2)+'<br /></span><a href="#" class="remove">Remove</a></p><div class="mobile">Available for <span>FREE</span> mobile delivery!</div></div></li>';
	...
	function createSeatImage(section,row,seat,cx,cy,i,pin) {
		...
		if (pin) {
			...
			seatImage.onload = function() {
				...
				$(".details-item").find(".img-list li:last-of-type img").attr("src",seatImageUrl);
			}*/
		}
	}
